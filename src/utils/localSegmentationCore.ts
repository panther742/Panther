/**
 * Panther Studio — Local Magic-Layer Segmentation CORE
 *
 * Dependency-free pixel analysis shared by the SERVER engine
 * (src/server/localSegmentationService.ts) and the BROWSER standalone engine
 * (src/lib/localPsdClient.ts) so the PSD maker's "magic layers" work
 * identically with or without the backend.
 *
 * Input: a downscaled interleaved RGBA buffer (max ~256px longest side).
 * Output: detected elements + background model.
 */
...
/**
 * Nearest-neighbor downscale (identical to Jimp's RESIZE_NEAREST_NEIGHBOR) so
 * the browser engine matches the server engine pixel-for-pixel.
 */
export function downscaleNearest(
  ...
  for (let y = 0; y < targetH; y++) {
    const srcY = Math.min(fullH - 1, Math.floor((y * fullH) / targetH));
    for (let x = 0; x < targetW; x++) {
      const srcX = Math.min(fullW - 1, Math.floor((x * fullW) / targetW));
      const si = (srcY * fullW + srcX) * 4;
...
/**
 * Run the magic-layer decomposition on an already-downscaled RGBA buffer.
 * Pure math — no DOM, no Node — works in browser and server identically.
 */
export function detectElementsFromPixels(
  data: Uint8Array | Uint8ClampedArray,
  w: number,
  h: number,
  maxElements: number = 15
): { elements: LocalDetectedElement[]; background: LocalBackgroundInfo } {
  const total = w * h;

  // 1. Build background color model from border pixels (2px frame, opaque only)
  ...
  // 2. Corner sampling for gradient detection (avg + stddev of each corner).
  ...
  // 4. Morphology: erode isolated noise only. NO dilation — dilation paints a
  ...
  // 5. Connected components (8-connectivity BFS)
  ...
  // 5b. Color-region splitting: elements that TOUCH each other (e.g. a person
  ...
  // 7. Smart merging: close/overlapping pieces of the same element
  ...
  // 8. Cap to element budget and build results
  ...
  for (let i = 0; i < Math.min(budget, merged.length); i++) {
    ...
    // c.minX/c.minY live in DOWNSCALED image coordinates, so normalize
  ...
  // 9. Background plate classification
